$(function(){
    ...
    $("#btn_toggle").bind("click", function(){
        if($(this).hasClass('on')){
           $(this).html('목록열기').removeClass('on').attr('aria-expanded', 'false').parents('.focus').find('ul').hide();
        }else{
           $(this).html('목록닫기').addClass('on').attr('aria-expanded', 'true').parents('.focus').find('ul').show();
        }                         
     });
